...
# this is mover_plot_script.py

def plot_mover_scores(mover_csv_path, output_prefix="mover_score_comparison"):
    """
    Create bar plots from CSV files for mover scores.
    
    Parameters:
    - mover_csv_path: Path to the mover scores CSV file
    ...
    mover_df = pd.read_csv(mover_csv_path)
    
    # Extract top_k values, which are in the first column
    topk_values = mover_df['top_k'].tolist()
    ...
    ax.set_ylabel('Movers Score', fontsize=16, fontweight='bold')
    ax.set_xticks(x)
    ...
    print(f"Movers score plot saved with prefix: {output_prefix}")


if __name__ == "__main__":
    import argparse
    
    parser = argparse.ArgumentParser(description="Generate plot from mover score CSV file")
    parser.add_argument("--mover", required=True, help="Path to mover score CSV file")
    parser.add_argument("--output", default="mover_score_comparison", help="Output file prefix")
    
    args = parser.parse_args()
    
    plot_mover_scores(args.mover, args.output)